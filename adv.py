...
#
# @modify : artfjqm2@skku.edu, evegreen96@skku.edu , chri0220@skku.edu, maya0707@skku.edu
# add function
# 1. dataloader
# 2. fairdrop
# 3. greedy
...
def main(args):
    G, adj, features, sensitive, communities, test_edges_true, test_edges_false, nodelist = load_dataset(args.dataset,
                                                                                                         'data')
    ...
    # drop edges fairly
    fairdrop = fairdropper(G, adj, args.epochs, args.device)
    ...
    for epoch in range(args.epochs):
        ...
        print("Epoch: [{0:d}/{1:d}]; Loss: {2:.3f}; Adv. loss: {3:.3f};".format((epoch + 1), args.epochs, loss, adv_loss))
    ...
    if args.greedy:
        ...
        # evaluation
        std = fair_link_eval(G, nodelist, communities, hidden_emb, sensitive_save, test_edges_true, test_edges_false,
                             args.dataset)
        print("Result below ------")
        print('METRIC\t' + ' '.join(
            '{0:>8}'.format(metric) for metric in ["auc", "ap", "modred", "dp", "true", "false", "fnr", "tnr"]))
